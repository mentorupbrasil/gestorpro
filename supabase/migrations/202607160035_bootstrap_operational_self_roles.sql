...
-- Bootstrap: tenant_admin com roles.manage + AAL2 pode autoatribuir/revogar
-- papéis operacionais. Continua proibido autoatribuir/revogar papéis privilegiados
-- (tenant_admin, restricted_support) — anti-autoelevação administrativa.
...
begin
  ...
  if membership_user_id = auth.uid()
     and role_code in ('tenant_admin', 'restricted_support') then
    raise exception 'cannot change own privileged membership roles' using errcode = '42501';
  end if;
